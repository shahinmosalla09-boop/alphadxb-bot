...
if __name__ == "__main__":
    ...
    try:
        ...
        if _os.path.exists("signals_journal.json"):
            with open("signals_journal.json") as _f:
                _j = _jj.load(_f)
            _n = len(_j.get("signals", []))
            send_message(ADMIN_BOT_TOKEN, ADMIN_CHAT_ID,
                         f"✅ Bot started — journal restored ({_n} signals loaded).\n"
                         f"Type /journal to see status.")
        else:
            send_message(ADMIN_BOT_TOKEN, ADMIN_CHAT_ID,
                         "⚠️ Bot started — journal NOT found in backup.\n"
                         "Send the signals_journal.json file here to restore.")
    except Exception as _e:
        print(f"[STARTUP] journal restore error: {_e}")
        send_message(ADMIN_BOT_TOKEN, ADMIN_CHAT_ID,
                     f"⚠️ Bot started — journal restore failed: {_e}\n"
                     "Send the signals_journal.json file here to restore.")
    ...
    # ----- Weekly journal archive + reset — Monday 00:00 UTC (04:00 Dubai) -----
    # Archives last week's journal to Telegram, resets for new trading week.
    def _weekly_archive():
        try:
            summary = public_signals.weekly_journal_archive(ADMIN_BOT_TOKEN, str(ADMIN_CHAT_ID))
            send_message(ADMIN_BOT_TOKEN, ADMIN_CHAT_ID,
                         f"🔄 <b>New trading week started!</b>\n\n"
                         f"{summary}\n\n"
                         f"Journal reset. Ready for the new week. 💪")
        except Exception as e:
            print(f"❌ weekly_archive error: {e}")

    schedule.every().monday.at("00:00").do(_weekly_archive)
